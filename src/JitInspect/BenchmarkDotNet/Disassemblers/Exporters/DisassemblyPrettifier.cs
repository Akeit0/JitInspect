...
internal static class DisassemblyPrettifier
{
    ...
    internal class Reference : Element
    {
        ...
        public Reference(string textRepresentation, string id, SourceCode source) : base(textRepresentation, source)
        {
            Id = id;
        }
    }
    ...
    internal class Label : Element
    {
        ...
        public Label(string label) : base(label, null)
        {
            Id = label;
        }
    }
    ...
    internal static IReadOnlyList<Element> Prettify(DisassembledMethod method, DisassemblyResult disassemblyResult, DisassemblyDiagnoserConfig config, string labelPrefix)
    {
        ...
        foreach (var instruction in map.SourceCodes)
            ...
            {
                prettified.Add(new(sharp.Text, sharp));
            }
            else if (instruction is MonoCode mono)
            {
                prettified.Add(new(mono.Text, mono));
            }
            else if (instruction is Asm asm)
            {
                // this IP is referenced by some jump|call, so we add a label
                if (addressesToLabels.TryGetValue(asm.InstructionPointer, out var label)) prettified.Add(new Label(label));

                if (asm.ReferencedAddress != null)
                {
                    var referencedAddress = asm.ReferencedAddress.Value;
                    ...
                    if (disassemblyResult.AddressToNameMapping.TryGetValue(referencedAddress, out var referencedName))
                    {
                        var comment = string.Empty;
                        if (asm.IsReferencedAddressIndirect) comment = "; " + referencedName;
                        prettified.Add(new(CodeFormatter.Format(asm, formatterWithGlobalSymbols, config.PrintInstructionAddresses, disassemblyResult.PointerSize, disassemblyResult.AddressToNameMapping) + comment, asm));
                        continue;
                    }
                }

                prettified.Add(new(CodeFormatter.Format(asm, formatterWithGlobalSymbols, config.PrintInstructionAddresses, disassemblyResult.PointerSize, disassemblyResult.AddressToNameMapping), asm));
            }
